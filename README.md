...
用户通过统一的身份认证入口进入系统，支持管理员（`admin`）与普通用户两种角色，登录后按权限路由至不同功能页面。

![登录界面](frontend/assets/images/b1.png)

### 管理员知识库管理

管理员可对景点、美食、住宿三类知识条目进行增删改查操作，支持按类型筛选、关键词搜索、分页浏览，以及结构化文本文件的批量导入解析。

![管理员后台](frontend/assets/images/b2.png)

### AI 对话界面（管理员 / 普通用户）

模型回答以 Markdown 格式渲染，支持多级标题、表格、引用块等富文本元素。文中 `[^N]` 引用标记可点击跳转至对应的知识库引用卡片，底部地图面板自动展示相关知识点的地理位置。左侧会话历史列表支持多会话管理与历史对话查看。

| 管理员对话 | 普通用户对话 |
|:---------:|:---------:|
| ![管理员对话](frontend/assets/images/b3.png) | ![用户对话](frontend/assets/images/b4.png) |

### 用户管理（管理员）

管理员可查看所有注册用户信息，页面顶部提供总用户数、今日/本周/本月新增、活跃用户数、管理员数量等核心统计指标，支持按关键词搜索、按角色筛选及分页浏览。

![用户管理](frontend/assets/images/b5.png)

管理员可通过"新增用户"弹窗直接创建账号，也可对现有用户执行角色变更（提升/降级管理员）和账户启用/禁用操作。
